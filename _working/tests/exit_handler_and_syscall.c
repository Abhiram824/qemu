...
asm(
    ...
    "mov x0, x16\n"  // Move target PC to x0 for HFI_ENTER


    "" HFI_ENTER(17, 16)  // Re-enter HFI mode with options in x17, target in x16
);

void debugg(uint64_t x) {
    char buffer[100];
    snprintf(buffer, sizeof(buffer), "[DEBUGG] Exit handler scaffold: target PC = 0x%llx\n", (unsigned long long)x);
    perror(buffer);
}

void mock_syscall(uint64_t regs[31]) {
    // copy regs x0-x5 and x8 and then make svc
    // note the additional clobbers x6-x7, x16-x17, x30, NZCV
    asm volatile(
        "mov x0, %[x0]\n"
        "mov x1, %[x1]\n"
        "mov x2, %[x2]\n"
        ...
        "svc #0\n"
        :
        : [x0] "r"(regs[0]), [x1] "r"(regs[1]), [x2] "r"(regs[2]), [x3] "r"(regs[3]), [x4] "r"(regs[4]), [x5] "r"(regs[5]), [x8] "r"(regs[8])
        : "x6", "x7", "x16", "x17", "x30", "cc");
}
...
void exit_handler(uint64_t regs[31]) {
    ...
    if (exit_reason == HFI_SYSCALL_REQUESTED) {
        // printf("[HARNESS] Syscall requested at PC: 0x%llx\n", (unsigned long long)exit_pc);
        // printf("hi\n");
        mock_syscall(regs);
    } else if (exit_reason == HFI_EXIT_CALLED) {
        // printf("[HARNESS] HFI exit called at PC: 0x%llx\n", (unsigned long long)exit_pc);
        asm volatile(
            "mov sp, %[hfi_end_sp]\n"
            "br %[hfi_end_addr]\n"
            :
