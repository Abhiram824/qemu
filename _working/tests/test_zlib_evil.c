...
void exit_handler(uint64_t regs[31])
{
    ...
    if (exit_reason == HFI_SYSCALL_REQUESTED) {
        ...
        printf("    - This was an expected fault. Returning to trusted code.\n");
        exit(0);
    }
}
...
void debug_marker() {
    printf("[[[[[[MARKER: test_zlib_evil started]]]]]]\n");
}

int test_zlib_evil(void)
{
    const char *lib_path = "./lib/libz_evil.so";
