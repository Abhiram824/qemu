...
void exit_handler(uint64_t regs[31]) {
    ...
    if (exit_reason == HFI_SYSCALL_REQUESTED) {
        ...
        printf("  -> A fault occurred inside HFI (Reason: %llu)\n", HFI_EXIT_STATE_GET_REASON(exit_state));
        printf(" -> Fault details:\n");
        printf("    - Operation: %llu\n", HFI_FAULT_STATE_GET_OPERATION(do_hfi_gfs()));
        printf("    - Reason: %llu\n", HFI_FAULT_STATE_GET_REASON(do_hfi_gfs()));
        printf("    - Region ID: %llu\n", HFI_FAULT_STATE_GET_REGION_ID(do_hfi_gfs()));
...
int test_zlib_success() {
    ...
    do_hfi_srb(2, 0);
    do_hfi_srm(2, ~0ULL);  // Global data mask for phase 1
