...
void test_hfi(int load_inside) {
    uint32_t len = 128;
    void *code = mmap(NULL, 4096, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_SHARED, -1, 0);
    void *data = mmap(NULL, 4096, PROT_READ|PROT_WRITE, MAP_SHARED, -1, 0);
    ...
    // now enter the code region with HFI_ENTER, which should succeed and allow us to execute code from the code region and read from the data region (if load_inside) or cause an exception if we try to read from outside the data region (!load_inside)
    asm volatile(
        "mov x1, %[code]\n"
        "mov x2, #0x1\n"
        "" HFI_ENTER(1, 2)
